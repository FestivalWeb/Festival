...
public class AdminAuthService {
    ...
    public AdminLoginResponse login(AdminLoginRequest request, HttpServletRequest httpRequest,  HttpServletResponse httpResponse) {
        ...
        if (opt.isPresent()) {
            ...
            // [추가] 대시보드 노출용 시스템 로그 (SystemLog) 추가
            adminLogWriter.logSystem(
                org.zerock.backend.entity.LogLevel.INFO,  // LogLevel import 필요
                "관리자 로그인(세션연장): " + adminUser.getUsername(),
                "LOGIN",
                httpRequest,
                200L,
                null
        ...
        // [추가] 대시보드 노출용 시스템 로그 (SystemLog) 추가
        adminLogWriter.logSystem(
            org.zerock.backend.entity.LogLevel.INFO, 
            "관리자 로그인(신규): " + adminUser.getUsername(),
            "LOGIN",
            httpRequest,
            200L,
            null
